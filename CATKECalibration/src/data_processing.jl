...
#
# Note: All imports are in the main CATKECalibration.jl module file
...
"""
    extract_midlatitude_section(fts; kwargs...)

Extract the mid-latitude section (-52°, 52°) from a field or field time series.
This is the standard latitude range for CATKE calibration.
"""
...
"""
    process_member_data(simdir, zonal_average; apply_dz_weighting=false, grid_regridder_path=nothing)
...
Arguments:
- `simdir`: Directory containing model output files
- `zonal_average`: Whether to take zonal averages
- `apply_dz_weighting`: Whether to apply dz-based vertical weighting
- `grid_regridder_path`: Path to JLD2 file containing target_grid and regridder
                         (defaults to examples/CATKE_calibration/4deg_grids_and_regridder.jld2)
"""
function process_member_data(simdir, zonal_average; apply_dz_weighting=false, grid_regridder_path=nothing)
    month_names = [:jan, :feb, :mar, :apr, :may, :jun, :jul, :aug, :sep, :oct, :nov, :dec]

    # Load target grid and regridder
    if isnothing(grid_regridder_path)
        grid_regridder_path = joinpath(pwd(), "examples", "CATKE_calibration", "4deg_grids_and_regridder.jld2")
    end

    target_grid, regridder = jldopen(grid_regridder_path, "r") do file
