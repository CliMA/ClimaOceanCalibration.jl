...
using ClimaOceanCalibration.OMIPSimulations: strait_transports, woa_to_teos10!, upper_orca_grid
...
using ClimaOceanCalibration.OMIPSimulations: jld2_output_part_paths, with_jld2, jld2_parts,
                       memoize_jld2_part,
...
function geo_panel!(fig, pos, data;
    ...
    # Must be set AFTER xlims!/ylims!: overriding the ticks rebuilds the
    # graticule, and doing so before valid limits exist corrupts the scene
    # bounding box that the limit-setters then read (inverted-limits error).
