# calibrate_catke_module.jl
# Main calibration script for CATKE parameter estimation using the CATKECalibration module
#
...
#
# The calibration uses the BatchedSlurmGCPBackend from the CATKECalibration module,
# which submits jobs that run multiple ensemble members in parallel on a single
# GCP a3mega node (8 GPUs per node).
#
# Prerequisites:
#   1. Add the CATKECalibration module to your project:
#      julia> using Pkg; Pkg.develop(path="CATKECalibration")
#
#   2. Generate monthly-averaged ECCO data:
#      julia --project examples/CATKE_calibration/average_ECCO_data.jl
#
# Usage:
#   julia --project examples/CATKE_calibration/calibrate_catke_module.jl
#
# The script will:
...
# Output directory
const output_dir = joinpath(pwd(), "calibration_runs", "catke_2yr_monthly_ecco_obscov_module")
mkpath(output_dir)
...
Cˢ_prior = constrained_gaussian("Cˢ_scaling", 1.0, 0.5, 0, Inf)     # Surface layer TKE production
Cᵘⁿ_prior = constrained_gaussian("Cᵘⁿ_scaling", 1.0, 0.5, 0, Inf)   # Unstable/convective mixing
Cᶜ_prior = constrained_gaussian("Cᶜ_scaling", 1.0, 0.5, 0, Inf)     # Stable/convective mixing
Cˢᵖ_prior = constrained_gaussian("Cˢᵖ_scaling", 1.0, 0.5, 0, Inf)   # Shear production
Cᵉc_prior = constrained_gaussian("Cᵉc_scaling", 1.0, 0.5, 0, Inf)   # TKE equation
...
# Uses functions from CATKECalibration module
...
n_batches = ceil(Int, ensemble_size / GPUS_PER_NODE)

# Save calibration metadata for model interface
...
# ============================================
hpc_kwargs = Dict(
    :time => 5 * 24 * 60,        # 5 days max runtime
    :partition => "a3mega",
...
# Model Interface
# ============================================
# The model interface file defines forward_model and observation_map functions
# It should use the CATKECalibration module functions
model_interface = joinpath(@__DIR__, "model_interface_module.jl")
