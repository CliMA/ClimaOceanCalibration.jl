...
#   * ClimaCalibrate.Backend.module_load_string(::BatchedSlurmGCPBackend)
#   * ClimaCalibrate.Backend.make_job_script(::BatchedSlurmGCPBackend, body; ...)
#   * ClimaCalibrate.Calibration.run_iteration(backend::BatchedSlurmGCPBackend, ...)

import ClimaCalibrate
isdefined(ClimaCalibrate, :Backend) || error(
    "batched_slurm_backend.jl requires ClimaCalibrate v0.3.x; " *
...
"""
    BatchedSlurmGCPBackend <: SlurmBackend
...
"""
struct BatchedSlurmGCPBackend <: SlurmBackend
    hpc_config::SlurmConfig
    job_records::Vector{JobInfo}
    gpus_per_node::Int
end

function BatchedSlurmGCPBackend(config::SlurmConfig; gpus_per_node = GPUS_PER_NODE)
    return BatchedSlurmGCPBackend(config, JobInfo[], gpus_per_node)
end

# ----------------------------------------------------------------------
# Cluster environment (a3mega, NVHPC 25.7, HPC-X 2.22.1, Julia 1.12.3)
# ----------------------------------------------------------------------
function ClimaCalibrate.Backend.module_load_string(::BatchedSlurmGCPBackend)
    return """
...
function ClimaCalibrate.Backend.make_job_script(
    backend::BatchedSlurmGCPBackend,
    job_body;
...
# ----------------------------------------------------------------------
function batch_backend(parent::BatchedSlurmGCPBackend, n_gpus::Integer)
    ...
    return BatchedSlurmGCPBackend(batch_cfg, parent.job_records, parent.gpus_per_node)
end
