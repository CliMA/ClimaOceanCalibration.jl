# model_interface.jl
# ClimaCalibrate hook-up for the ORCA CATKE+GM calibration, implementing the
# v0.3.0 AbstractModelInterface (3-arg) API directly.
#
# Variables expected to be defined before this file is included:
#   - output_dir           (calibration top-level dir)
# The remaining metadata (output_dim, ensemble_size, latitude_range, z_min,
# sampling_length, simulation_length, calibrated parameter names, woa_file,
# filename_prefix, staging_dir, use_gm, with_ice_dynamics) are loaded from
# calibration_metadata.jld2 when not already defined by the controller.
#
# Both the parent driver (calibrate_catke_gm.jl) and the child forward-model
# script (see batched_job_body in batched_slurm_backend.jl) set `output_dir`
# before including this file, so the @isdefined check below passes in both
# contexts.
...
"""
    forward_model(::CATKEGMInterface, iteration, member)

Read the per-member TOML parameter file, split keys into CATKE / GM
scalings, and dispatch to `run_CATKE_GM_calibration_orca`.
"""
function ClimaCalibrate.forward_model(::CATKEGMInterface, iteration, member)
...
"""
    observation_map(::CATKEGMInterface, iteration)
...
"""
function ClimaCalibrate.observation_map(::CATKEGMInterface, iteration)
...
"""
    analyze_iteration(::CATKEGMInterface, ekp, g_ensemble, prior, output_dir, iteration)

Save EKP diagnostics and render per-member figures vs WOA.
"""
function ClimaCalibrate.analyze_iteration(::CATKEGMInterface, ekp, g_ensemble, prior, calib_output_dir, iteration)
