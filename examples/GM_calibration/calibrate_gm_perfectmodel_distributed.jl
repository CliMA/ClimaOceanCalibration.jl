...
@everywhere begin
    ...
    const output_dir = joinpath(pwd(), "calibration_runs", "gm_$(simulation_length)year_perfectmodel_distributed$(zonal_average ? "_zonalavg" : "")")
...
obs_paths = abspath.(glob("*750.0_750.0*$(sampling_length)yearsample*", joinpath("calibration_data", "synthetic_observations")))

calibration_target_obs_path = abspath(joinpath("calibration_data", "synthetic_observations", "halfdegree_RiBased_750.0_750.0_1992_15year_5yearsample_advectiveGM_multiyearjra55_calibrationsamples"))

Y = hcat(process_member_data.(obs_paths, no_tapering, zonal_average)...)
...
Y_obs = Observation(Dict("samples" => process_member_data(calibration_target_obs_path, taper_interior_ocean, zonal_average),
