...
- `κ_skew`, `κ_symmetric`: GM/Redi diffusivities. Defaults: `250`, `100`. (Legacy shorthand
   for `gm_parameters`; merged in when non-`nothing`.)
- `catke_parameters::NamedTuple`: structured overrides for CATKE; accepts
   `mixing_length = (; …)` and `tke_equation = (; …)` to reach `CATKEMixingLength` /
   `CATKEEquation` fields, plus any top-level `CATKEVerticalDiffusivity` fields. Empty
   → upstream defaults.
- `gm_parameters::NamedTuple`: overrides for `IsopycnalSkewSymmetricDiffusivity`.
   Nested NamedTuples recurse, so `(; slope_limiter = (; max_slope = 0.01))` works.
   Default: `(; κ_skew = 250, κ_symmetric = 100)`. `κ_skew == 0` or
   `κ_symmetric == 0` disables the GM term entirely.
...
- `monthly_averaging_interval`: window for the `_monthly_fields` (3-D T/S/b) and
   ...
   `365days/12` (one climatological month).
...
# (NamedTuple) value, and remaining keys are gathered into a `top` NamedTuple
# that is applied to the top-level struct.
function _split_overrides(nt::NamedTuple, nested_keys::Tuple)
    rest_keys = Tuple(k for k in keys(nt) if !(k in nested_keys))
    rest_vals = Tuple(nt[k] for k in rest_keys)
    return (; (rest_keys .=> rest_vals)...)
end
...
#
# `catke_parameters` is a structured NamedTuple of overrides for the
# CATKE pieces:
#   (; mixing_length = (; Cᵇ = …, Cˢ = …),
#      tke_equation  = (; Cᵂu★ = …),
#      <any CATKEVerticalDiffusivity top-level field>...)
# `gm_parameters` is a NamedTuple of overrides for the
# `IsopycnalSkewSymmetricDiffusivity`; nested NamedTuples recurse into
# sub-structs (e.g., `slope_limiter`).
function omip_closure(vertical_closure::Symbol;
                      ...
                      # they are merged into the corresponding NamedTuple above.
                      κ_skew = nothing,
                      κ_symmetric = nothing,
                      Cᵇ = nothing,
                      Cᵂu★ = nothing)

    # Merge legacy kwargs into the structured NamedTuples so callers using the
    # old interface (e.g., examples/OMIP_GCP/launch.sh) keep working.
    if !isnothing(Cᵇ)
        ml_extra = (; Cᵇ)
        existing_ml = get(catke_parameters, :mixing_length, (;))
        catke_parameters = merge(catke_parameters, (; mixing_length = merge(existing_ml, ml_extra)))
    end
    if !isnothing(Cᵂu★)
        tke_extra = (; Cᵂu★)
        existing_tke = get(catke_parameters, :tke_equation, (;))
        catke_parameters = merge(catke_parameters, (; tke_equation = merge(existing_tke, tke_extra)))
    end
    if !isnothing(κ_skew)
        gm_parameters = merge(gm_parameters, (; κ_skew))
    end
    if !isnothing(κ_symmetric)
        gm_parameters = merge(gm_parameters, (; κ_symmetric))
    end

    primary, background = if vertical_closure == :catke
        ml_overrides  = get(catke_parameters, :mixing_length, (;))
        tke_overrides = get(catke_parameters, :tke_equation, (;))
        catke_top     = _split_overrides(catke_parameters, (:mixing_length, :tke_equation))
    ...
    # Build IsopycnalSkewSymmetricDiffusivity from the upstream defaults
    # (κ_skew=250, κ_symmetric=100) then apply any nested overrides.
    # Preserve the "0 ⇒ no eddy closure" sentinel.
    eddy = let
        κ_sk = get(gm_parameters, :κ_skew, 250)
        κ_sy = get(gm_parameters, :κ_symmetric, 100)
        if κ_sk == 0 || κ_sy == 0 || κ_sk === nothing || κ_sy === nothing
            nothing
        else
